...
1. **Studienpopulation**: `{python} len(df)` Patienten, ausgeglichene Geschlechterverteilung
2. **Altersverteilung**: Mittelwert `{python} f"{df['age'].mean():.1f}"` Jahre
3. **Response-Rate gesamt**: `{python} f"{df['response'].mean()*100:.1f}"`%
4. **BMI-Blutdruck-Korrelation**: r = `{python} f"{df['bmi'].corr(df['blood_pressure_sys']):.2f}"`
:::
